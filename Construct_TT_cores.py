...
def get_TT_svd_transposed(list_T, TT_ranks): # Returns a list of CP factors
    return [  matrix_product_state_transposed(T.astype('f'), TT_ranks, verbose=False, type_svd = 'partial_svd') for T in list_T]
...
def compute_TT_cores_transposed(list_data,TT_ranks):
    print("Get TT-svd with factors transposed")
    t0 = time.time()
    list_TT_cores =  get_TT_svd_transposed(list_data, TT_ranks)
    t1 = time.time()-t0
    return list_TT_cores, t1
