...
def get_single_tensorial_kernel1(factors_X, factors_Y, rank, gamma):
    ...
    M1 = np.squeeze(factors_X[0])
    M2 = np.squeeze(factors_Y[0])
    proj_M1 = projector_matrix(M1)
    proj_M2 = projector_matrix(M2)
    norm_1 = np.linalg.norm(proj_M1)
    norm_2 = np.linalg.norm(proj_M2)
    K1 = (np.linalg.norm(proj_M1/norm_1 - proj_M2/norm_2, 'fro'))**2
    ...
    norm_1 = np.linalg.norm(proj_M1)
    norm_2 = np.linalg.norm(proj_M2)
    K2 =(np.linalg.norm(proj_M1/norm_1 - proj_M2/norm_1, 'fro'))**2
    
    prod = 1
    for q in range(1, Q-1):
        G1 = factors_X[q]
        G2 = factors_Y[q]
        proj_G1 =projector(G1)
        proj_G2 =projector(G2)
        norm1 = np.linalg.norm(proj_G1)
        norm2 = np.linalg.norm(proj_G2)
        prod = prod*(np.linalg.norm(proj_G1/norm1 - proj_G2/norm2)**2)
...
def kernel_mat(X1_data, X2_data, rank, gamma):#X1_data : list of data factors; X1_data[0]: factors of the 1st data
    ...
    for i, L1 in enumerate(X1_data):
        for j, L2 in enumerate(X2_data):
              K.append(get_single_tensorial_kernel1(L1, L2, rank, gamma))
    K = np.asarray(K)
