...
from kernel_optimized import  construct_list_projectors, get_proj_single, get_base_single, get_base_data, get_proj_data, test_proj, test_proj_matrix, normaliser
...
for n in tqdm.tqdm(range(10)):
    ...
    #list_projectors, list_TT_cores, list_labels, time_TT, time_proj= get_projectors(file_extended, TT_ranks, bruit = 'True')
    list_projectors, list_labels, time_TT, time_proj, list_TT_cores = get_projectors_ameliorated(file_extended, TT_ranks)
    list_time_TT.append(time_TT)
    list_time_const_proj.append(time_proj)
    J_train =  precomputed_kernel_matrix(list_projectors, list_projectors)
    t0 = time.time()
